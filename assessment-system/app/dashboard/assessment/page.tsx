...
export default function AssessmentPage() {
  ...
  const handleImageChange = (topicId: string, files: FileList | null) => {
    ...
    setPreviews(prev => ({ ...prev, [topicId]: [...(prev[topicId] || []), ...urls].slice(0, 5) }))
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const topicId of selected) {
        ...
        const topicImages = images[topicId] || []
        for (const file of topicImages) {
          const path = `${user.id}/${assessment.id}/${result.id}/${Date.now()}-${file.name}`
          ...
          const { data: { publicUrl } } = supabase.storage
            .from('assessment-images')
            .getPublicUrl(path)
          await supabase.from('assessment_images').insert({
            result_id: result.id,
            image_url: publicUrl,
            file_name: file.name,
          })
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex gap-2 mb-6">
        {['เลือกหัวข้อ', 'กรอกข้อมูล', 'ตรวจสอบ'].map((s, i) => (
          <div key={i} className={`flex-1 py-2 px-3 rounded-xl text-xs text-center font-medium
          ...
          <div className="space-y-4 mb-4">
            {selectedTopics.map(t => (
              <div key={t.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                ...
                />
                <div>
                  <label className="block text-xs text-gray-500 mb-2">แนบรูปภาพ (สูงสุด 5 รูป)</label>
                  <label className="block w-full border-2 border-dashed border-gray-200 rounded-xl p-4 text-center cursor-pointer hover:border-blue-300 hover:bg-blue-50 transition-colors">
                    <input
                      type="file"
                      accept="image/*"
                      multiple
                    ...
                    <div className="text-xs text-gray-400">คลิกเพื่อเลือกรูป หรือลากมาวาง</div>
                  </label>
