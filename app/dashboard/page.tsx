...
export default function DashboardPage() {
  const [stats, setStats] = useState({ assessments: 0, topics: 0, reports: 0, users: 0 })
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      setStats({ assessments: a.count||0, topics: t.count||0, reports: r.count||0, users: u.count||0 })
    }
  ...
  const statusLabel: Record<string, string> = { draft:'ร่าง', submitted:'ส่งแล้ว', reviewed:'ตรวจแล้ว' }
  const statusColor: Record<string, string> = { draft:'bg-yellow-50 text-yellow-700', submitted:'bg-blue-50 text-blue-700', reviewed:'bg-green-50 text-green-700' }
  const cards = [
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
        <h1 className="text-sm font-medium text-gray-900">แดชบอร์ด</h1>
        <div className="flex gap-3">
          <Link href="/dashboard/assessment" className="text-xs text-blue-600">สร้างประเมิน</Link>
          <Link href="/dashboard/topics" className="text-xs text-gray-500">หัวข้อ</Link>
          <Link href="/dashboard/report" className="text-xs text-gray-500">รายงาน</Link>
        </div>
      </div>
      <div className="p-6">
